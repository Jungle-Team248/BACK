...
export default class Game {
    ...
    startGame() {
        ...
        setTimeout(async ()=>{
            ...
            if (this.started === false) {
                // gameStart를 위해 mafia, turn 등 정해야하나, 누군가의 exit으로 인해 게임이 종료됨
                return null;
            }
            this.setGameTurn();
            this.drawMafia();
            ...
            setTimeout(()=>{
                // 비동기 처리로 대기하는 사이 게임이 비정상 종료될 경우를 대비.
                if (this.started === false) {
                    // openTurn 호출해야하나, 누군가의 exit으로 인해 게임이 종료됨
                    return null;
                }
    ...
    openTurn() {
        // 비동기 처리로 대기하는 사이 게임이 비정상 종료될 경우를 대비.
        if (this.started === false) {
            // openTurn 받았으나, 누군가의 exit으로 인해 게임이 종료됨
            return null;
        }
    ...
    nightWork(user, submit) {
        ...
        // 플레이어 전원이 투표 완료한 경우
        if ( this.nightDone >= this.turnQue.length ) {  
    ...
    exitGame(userId) { 
        ...
        if (!this.started){
            ...
            if (this.mafia == userId) {
                // mafia 나감; 바로 시민 승리 게임 close 해야함
                nightData.win = "citizen";
